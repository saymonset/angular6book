...
Promise<boolean> | boolean {
	if (route.params["mode"] == "create") {
		...
		let responses: [string, () => void][] = [["Yes", () => resolve(true)], 
		               ["No", () => resolve(false)]];
		this.messages.reportMessage(new Message("Do you accept the terms & conditions?",
								false, responses));
	});
	} else {
		return true;
...
canActivateChild(route: ActivatedRouteSnapshot, state: RouterStateSnapshot):
	Promise<boolean> | boolean {
		if (route.url.length > 0 && route.url[route.url.length - 1].path == "categories") {
			...
			let responses: [string, (string) => void][] = [["Yes", () => resolve(true)],
															["No ", () => resolve(false)]
														  ];
